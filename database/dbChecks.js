...
//CONDITIONALS
async function EmptyNodeExists(campId) {
...
async function CanAddNode(campId, userId) {
  ...
  const q = await db.query(
    ...
    SELECT
        camps.finished,
        nodes_0.creator_id AS last_node_creator,
  ...
  if (diffInMinutes < 20) throw new Error('Cant add node. Another player is currently writing');
